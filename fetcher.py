...
def fetch_vix_cboe() -> float | None:
    ...
    # Fallback: Tiingo VIXCLS (FRED series via Tiingo)
    try:
        history = tiingo_get_history("VIXCLS", days=3)
        if history:
            vix = history[-1]
            if 10 <= vix <= 80:
                print(f"[FETCHER] VIX: {vix} via Tiingo VIXCLS")
                return vix
    except Exception as e:
        print(f"[FETCHER] Tiingo VIX error: {e}")

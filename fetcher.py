    ...
    if data.get("earnings_date_raw") and expiry_raw:
        try:
            ...
            if data.get("earnings_is_past"):
                # Post-earnings play — gap is days AFTER last earnings
                days_ago = data.get("days_since_earnings", 0)
                if days_ago <= 14:
                    if gap >= 14:
                        data["expiry_timing_label"] = f"Post-earnings play — reported {days_ago}d ago, {gap}d to expiry ✅"
                        data["expiry_timing_emoji"] = "✅"
                    elif gap >= 7:
                        data["expiry_timing_label"] = f"Post-earnings — reported {days_ago}d ago, {gap}d to expiry ⚠️ tight"
                        data["expiry_timing_emoji"] = "⚠️"
                    else:
                        data["expiry_timing_label"] = f"Post-earnings — {gap}d to expiry, very tight"
                        data["expiry_timing_emoji"] = "❌"
                else:
                    data["expiry_timing_label"] = f"Earnings {days_ago}d ago — momentum play, {gap}d to expiry"
                    data["expiry_timing_emoji"] = "⚠️"
            else:
                # Pre-earnings play

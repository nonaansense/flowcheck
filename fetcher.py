...
_sector_cache = {}
_market_cache = {}
_MARKET_TTL   = 180  # 3 min market cache — ensures fresh VIX on each alert
...
# fetch_vix_cboe removed — VIX logic is inline in fetch_market_conditions
...
def fetch_market_conditions() -> dict:
    ...
    if not vix_val:
        for cboe_url in [
            ...
            try:
                r = requests.get(cboe_url, timeout=6,
                ...
                if r.status_code == 200:
                    ...
                    if result:
                        ...
                        if p and 10 <= float(p) <= 80:
                            vix_val = round(float(p), 1)
                            print(f"[FETCHER] VIX: {vix_val} via Yahoo")
                            break
                print(f"[FETCHER] VIX source {cboe_url[:40]} returned {r.status_code}")
            except Exception as e:
                print(f"[FETCHER] VIX source error: {str(e)[:60]}")
    ...
    if vix_val:
        ...
        print("[FETCHER] VIX unavailable — all sources failed, using 18.0 neutral estimate")
...
def fetch_trade_data(trade, flow_premium=None) -> dict:
    ...
    data = {
        ...
        # Fill aggression fields (from vision parser)
        "ask_pct":None,"bid_pct":None,"multi_pct":None,
        "fill_type":None,"fill_emoji":None,"fill_label":None,
        # Breakout detection
    ...
    # Calculate fill aggression from ask_size/bid_size/mid_size
    ...
    total_size = ask_size + bid_size + mid_size

    if total_size > 0:
        ask_pct = round((ask_size / total_size) * 100, 1)
        bid_pct = round((bid_size / total_size) * 100, 1)
        data["ask_pct"]   = ask_pct
        data["bid_pct"]   = bid_pct
        data["multi_pct"] = multi_pct

        # Classify fill aggression
        if ask_pct >= 90:
            data["fill_type"]  = "FULL_ASK"
            data["fill_emoji"] = "🚨"
            data["fill_label"] = f"100% at ask ({ask_size:,} contracts) — maximum aggression"
        elif ask_pct >= 70:
            data["fill_type"]  = "MOSTLY_ASK"
            data["fill_emoji"] = "✅"
            data["fill_label"] = f"{ask_pct}% at ask — aggressive buyer"
        elif ask_pct >= 40:
            data["fill_type"]  = "MIXED"
            data["fill_emoji"] = "⚠️"
            data["fill_label"] = f"{ask_pct}% ask / {bid_pct}% bid — mixed fill, less conviction"
        else:
            data["fill_type"]  = "MOSTLY_BID"
            data["fill_emoji"] = "❌"
            data["fill_label"] = f"{bid_pct}% at bid — passive fill, possible hedge or close"

        if multi_pct and float(multi_pct) > 20:
            data["fill_label"] += f" ⚠️ {multi_pct}% multi-leg (spread)"
            data["fill_type"]   = "SPREAD_LEG"
    else:
        # No size data — infer from tweet text if possible
        tweet_text = trade.get("raw_text", "").lower()
        if "above ask" in tweet_text or "ask buyer" in tweet_text or "ask sweep" in tweet_text:
            data["fill_type"]  = "FULL_ASK"
            data["fill_emoji"] = "🚨"
            data["fill_label"] = "Filled above/at ask (from tweet text)"
        else:
            data["fill_type"]  = "UNKNOWN"
            data["fill_emoji"] = "❓"
            data["fill_label"] = "Fill type unknown — check Bullflow screenshot"
    ...
    # ── Breakout-at-resistance detection ────────────────────────────────
    otm  = data.get("otm_pct") or (trade.get("otm") and float(trade.get("otm")))
    dte  = data.get("days_to_expiry")
    has_catalyst = bool(data.get("earnings_date") and not data.get("earnings_is_past"))

    if otm is not None and dte is not None:
        is_near_atm      = abs(float(otm)) < 2.0
        is_short_dated   = dte < 21
        near_round_number = False

        # Check if strike is a round number (resistance level)
        if strike:
            try:
                s = float(strike)
                near_round_number = (s % 5 == 0)  # Ends in 0 or 5
            except:
                pass

        if is_near_atm and is_short_dated and not has_catalyst:
            data["is_breakout_bet"]    = True
            data["breakout_emoji"]     = "⚠️"
            data["breakout_label"]     = (
                f"Breakout bet — {otm:.1f}% OTM, {dte}d expiry, no catalyst. "
                f"{'Near round number resistance. ' if near_round_number else ''}"
                f"Breakouts fail ~60% — consider straddle."
            )
            print(f"[FETCHER] ⚠️ Breakout bet detected: OTM={otm}%, DTE={dte}d, round={near_round_number}")
        elif is_near_atm and not has_catalyst:
            data["is_breakout_bet"]    = True
            data["breakout_emoji"]     = "⚠️"
            data["breakout_label"]     = (
                f"Near-ATM with no catalyst — momentum/breakout play. "
                f"Consider waiting for confirmation before entry."
            )
        else:
            data["is_breakout_bet"]    = False
            data["breakout_emoji"]     = ""
            data["breakout_label"]     = ""
    else:
        data["is_breakout_bet"] = False
    ...
    print(f"[FETCHER] Done: price=${stock_price}, earn={data['earnings_date']}, OTM={data['otm_pct']}%, DTE={data['days_to_expiry']}, breakout={data['is_breakout_bet']}")
    return data

        ...
        if r.status_code == 200:
            ...
            if isinstance(articles, list):
                ...
                for a in articles:
                    ...
                    # Must mention ticker specifically
                    mentions_ticker = ticker.lower() in combined
                    if not mentions_ticker:
                        continue

                    # Skip generic market news unless ticker is primary subject
                    is_generic = any(kw in headline for kw in generic_keywords)
                    if is_generic and not headline.startswith(ticker.lower()):
                        # Only include if ticker appears first in headline
                        ticker_pos = headline.find(ticker.lower())
                        if ticker_pos > 30:  # Ticker mentioned late = not primary subject
                            continue

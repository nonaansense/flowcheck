...
def build_eod_sms(analyses: list) -> str:
    ...
    Loads from disk to get full day count even after restarts.
    """
    import json
    now_et       = datetime.now(ZoneInfo("America/New_York"))
    today_str    = now_et.strftime("%Y-%m-%d")
    ...
    # Try loading full day from disk first
    all_analyses = list(analyses)  # in-memory
    try:
        with open("/tmp/flowcheck_analyses.json") as f:
            saved = json.load(f)
        if saved.get("date") == today_str:
            saved_list = saved.get("analyses", [])
            # Merge — avoid duplicates by id
            existing_ids = {a.get("id") for a in all_analyses}
            for a in saved_list:
    ...
    today_all = [a for a in all_analyses if a.get("date") == today_str]

...
async def backtest_endpoint(request: Request):
    ...
    try:
        ...
        if not trade or not trade.get("ticker"):
            return {"status":"error","reason":"Could not extract trade from tweet"}
        ...
        return {
            ...
            "historical": {
                ...
                "spy_trend":   data.get("market",{}).get("spy_trend"),
                "dte":         data.get("days_to_expiry"),
            },
            "score": {
        ...
        import traceback
        return {"status":"error","reason":str(e),"trace":traceback.format_exc()[:500]}

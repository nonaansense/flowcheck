...
async def startup():
    try:
        ...
        scheduler.start()
        print("[SCHEDULER] Started: 8:00AM pre-market | 4:15PM EOD | 4:30PM EOD SMS | every 5min technical scan")
    except Exception as e:
...
async def process_alert(tweet: str, tweet_url: str):
    try:
        ...
        data = fetch_trade_data(trade, flow_premium=flow_premium)
        ...
        if result.get("verdict") in ("WATCH", "TRADE"):
            add_to_watchlist(ticker, trade, result, data)
    ...
    return {
        ...
        "tickers": {
            t: {
                ...
                "added_ago": f"{int((time.time()-e['added'])/60)}min ago",

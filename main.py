...
def check_polygon_health():
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            send_sms(f"⚠️ Polygon API error {r.status_code} — scanner may be affected")
    except Exception as e:
        send_sms(f"⚠️ Polygon unreachable: {str(e)[:60]}")
...
async def startup():
    try:
        ...
        scheduler.add_job(remind_open_journal_trades,
        ...
        scheduler.add_job(lambda: send_theta_calendar(send_sms),
                          "cron", day_of_week="mon", hour=8, minute=5, id="theta_calendar")
        scheduler.add_job(check_polygon_health,
...
async def journal_page():
    """Full trade journal as HTML table — viewable on phone, downloadable as CSV."""
    try:
    ...
    if open_t:
        open_section = (
            ...
            "<tr><th>Ticker</th><th>Contract</th><th>Expiry</th><th>Qty</th>"
            "<th>Entry $</th><th>Cost</th><th>Entry Time</th><th>FlowCheck</th><th>Note</th></tr>"
            + rows_open +
            "</table></div>"
        )
    ...
    if closed_t:
        closed_section = (
            f"<h2>Closed Trades ({len(closed_t)})</h2>"
            "<div class='scroll'><table>"
            "<tr><th>Ticker</th><th>Contract</th><th>Expiry</th><th>Qty</th>"
...
function downloadCSV(){{
  ...
  a.download='flowcheck_journal.csv';
  a.click();
}}
</script>
</body></html>"""

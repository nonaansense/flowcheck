...
async def journal_page(account: str = None):
    ...
    if open_t:
        open_section = (
            ...
            "<tr>" + acc_th +
            "<th>Ticker</th><th>Contract</th><th>Expiry</th><th>Qty</th>"
    ...
    if multi_account:
        ...
        active_all = "" if account else "background:#3b82f6"
        ...
        for aid, acc in accounts.items():
            ...
            tabs_html += f'<a href="{base_url_j}?account={aid}" style="padding:6px 14px;border-radius:6px;background:{active};color:#fff;text-decoration:none;font-size:13px">{acc.get("name",aid)}</a>'
        tabs_html += "</div>"

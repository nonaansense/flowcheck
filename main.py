...
async def startup():
    try:
        ...
        scheduler.add_job(poll_commands,
                          "interval", seconds=3, id="telegram_commands")
...
def build_sms(trade: dict, data: dict, result: dict,
    ...
    if mins_to_close <= 0:
        if is_leap:
            ...
            time_warning = f"🌙 AFTER-HOURS FLOW ({dte_val}d DTE) — stealth buy, expect overnight/pre-market move"
    elif mins_to_close <= 30:
        if is_leap:
            time_warning = f"⏰ Late day LEAP ({mins_to_close}min left) — long-dated, no urgency signal"
        else:
            time_warning = f"🎯 LATE DAY FLOW ({mins_to_close}min left, {dte_val}d DTE) — stealth buy, avoiding copycats"
    else:
        time_warning = None
    ...
    if option_price:
        ...
        sizing     = calc_position_size(float(option_price), verdict,
                                        win_rate=win_rate, score=final_score)
        sizing_str = format_sizing_for_sms(sizing, float(option_price))
        if sizing_str:

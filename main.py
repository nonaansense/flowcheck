...
# ─────────────────────────────────────────
# PERSISTENCE — survive Railway restarts
# ─────────────────────────────────────────
ANALYSES_FILE = "/tmp/flowcheck_analyses.json"

def save_analyses():
    """Save today's analyses to disk so EOD summary survives restarts."""
    try:
            ...
            try:
                entry = {
                    ...
                    "trade": {
                        ...
                        "option_type": a["trade"].get("option_type"),
                        "expiry":      a["trade"].get("expiry"),
                    },
                    "result": {
                        "verdict":     a["result"].get("verdict"),
                        "final_score": a["result"].get("final_score"),
                        "raw_score":   a["result"].get("raw_score"),
                        "one_liner":   a["result"].get("one_liner",""),
                        "improvements":a["result"].get("improvements",[]),
                    },
                    "data": {
                        "stock_price":       a["data"].get("stock_price"),
                        "open_interest":     a["data"].get("open_interest"),
                        "days_to_expiry":    a["data"].get("days_to_expiry"),
                        "earnings_date":     a["data"].get("earnings_date"),
                        "fill_type":         a["data"].get("fill_type"),
                        "fill_label":        a["data"].get("fill_label"),
                        "expiry_timing_label":a["data"].get("expiry_timing_label"),
                    },
                    "pattern": a.get("pattern",{}),
                    "macro":   {
                        "advisory": a.get("macro",{}).get("advisory",""),
                        "max_impact": a.get("macro",{}).get("max_impact","NONE"),
                    }
                }
...
def load_analyses():
    """Load today's analyses from disk on startup."""
    try:
        today = datetime.now(ZoneInfo("America/New_York")).date().isoformat()
        ...
        if data.get("date") == today:
            ...
            print(f"[PERSIST] Loaded {len(loaded)} analyses from disk for {today}")
            return loaded
        else:
            print(f"[PERSIST] Stale data from {data.get('date')} — starting fresh")
...
# IN-MEMORY STORES
# ─────────────────────────────────────────
analyses      = load_analyses()  # Restored from disk on startup
...
def is_duplicate(ticker: str, expiry: str, strike: str) -> bool:
    ...
    now  = time.time()
    prev = [t for t in seen_tickers.get(key, []) if now - t < 120]  # 2 min
...
async def process_trade(tweet_text: str, trade: dict, image_url: str = None, tweet_url: str = None):
    ...
    try:
        ...
        save_analyses()  # Persist to disk so EOD summary survives restarts
...
def build_alert_sms(trade, result, data, macro, pattern, analysis_id, tweet_url=None):
    ...
    def cap_at_word(text: str, limit: int = 100) -> str:
    ...
    one_liner   = cap_at_word(result.get("one_liner", ""), 120)
    top_imp     = cap_at_word((result.get("improvements") or [""])[0], 120)
    mkt         = data.get("market", {})
    ...
    # Add days to expiry context to header
    dte = data.get("days_to_expiry")
    if dte is not None:
        if dte <= 1:
            dte_str = f" [⚠️ {dte}-DTE]"
        elif dte > 300:
            dte_str = f" [LEAP {dte}d]"
        else:
            dte_str = f" [{dte}d]"
    else:
        dte_str = ""
    ...
    if len(body) > max_body:
        # Truncate at last COMPLETE line that fits — never mid-line
        truncated = []
        running   = 0
        for line in body_lines:
            line_len = len(line) + 1  # +1 for newline
            if running + line_len > max_body:
                break  # Stop before adding a line that would overflow
...
async def analysis_detail(analysis_id: int):
    ...
    # Build tweet link HTML safely outside f-string to avoid quote conflicts
    if a.get("tweet_url"):
        tweet_link_html = f'''<a href="{a["tweet_url"]}" target="_blank" style="display:inline-block;margin-top:10px;padding:6px 14px;border-radius:6px;background:rgba(96,165,250,0.1);border:1px solid rgba(96,165,250,0.3);color:#60A5FA;font-size:12px;font-family:monospace;text-decoration:none">🐦 View original tweet →</a>'''
    else:
        tweet_link_html = ""
...
async def test_telegram():
    """Send a test message directly to Telegram — bypasses full analysis pipeline."""
    ...
    msg = (
        ...
        "\nIf you see this, the pipeline is live."
    )
    success = send_telegram(msg, token, chat_id)
    return {"status": "sent" if success else "failed"}


@app.get("/test-av")
async def test_av():
    """Test Alpha Vantage API key is working."""
    from fetcher import av_get, av_key
    if not av_key():
        return {"status": "error", "reason": "ALPHA_VANTAGE_KEY not set"}
    data = av_get({"function": "GLOBAL_QUOTE", "symbol": "AAPL"})
    if data and data.get("Global Quote"):
        price = data["Global Quote"].get("05. price", "?")
        return {"status": "ok", "AAPL_price": price}
    return {"status": "error", "reason": "Alpha Vantage returned no data", "raw": str(data)[:200]}


@app.get("/test-finnhub")
async def test_finnhub():
    """Test Finnhub API key is working."""
    from fetcher import fh_get, fh_key
    if not fh_key():
        return {"status": "error", "reason": "FINNHUB_API_KEY not set"}
    data = fh_get("/quote", {"symbol": "AAPL"})
    if data and data.get("c", 0) > 0:
        return {"status": "ok", "AAPL_price": data["c"], "source": "Finnhub"}
    return {"status": "error", "reason": "Finnhub returned no data", "raw": str(data)[:200]}


@app.get("/test-tiingo")
async def test_tiingo():
    """Test Tiingo API key — checks SPY price and history."""
    from fetcher import tiingo_get_price, tiingo_get_history, tiingo_key
    if not tiingo_key():
        return {"status": "error", "reason": "TIINGO_API_KEY not set"}
    spy_price   = tiingo_get_price("SPY")
    spy_history = tiingo_get_history("SPY", days=7)
    vix_history = tiingo_get_history("VIXCLS", days=7)
    return {
        "status":      "ok" if spy_price else "error",
        "SPY_price":   spy_price,
        "SPY_history": spy_history[-3:] if spy_history else [],
        "VIX_history": vix_history[-3:] if vix_history else [],
        "source":      "Tiingo"
    }

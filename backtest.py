...
def get_historical_price(ticker: str, dt: datetime) -> float | None:
    ...
    # Format datetime for Polygon
    date_str = dt.strftime("%Y-%m-%d")

    # Try to get the specific minute bar
    from_str = (dt - timedelta(minutes=5)).strftime("%Y-%m-%dT%H:%M:%S")
    to_str   = (dt + timedelta(minutes=5)).strftime("%Y-%m-%dT%H:%M:%S")

    data = polygon_get(
        f"/v2/aggs/ticker/{ticker.upper()}/range/1/minute/{from_str}/{to_str}",
        {"adjusted": "true", "sort": "asc", "limit": 10}
    )
    if data and data.get("results"):
        # Get closest bar to the tweet time
        target_ts = int(dt.timestamp() * 1000)
        closest = min(data["results"], key=lambda x: abs(x["t"] - target_ts))
        price = round(float(closest["c"]), 2)
        print(f"[BACKTEST] {ticker} at {dt.strftime('%H:%M')}: ${price} (historical)")
        return price
    ...
    if data2 and data2.get("results"):
        price = round(float(data2["results"][0]["c"]), 2)
        print(f"[BACKTEST] {ticker} on {date_str}: ${price} (daily close)")
        return price

    return None

def get_historical_vix(dt: datetime) -> float | None:
    """Get VIX at a specific datetime."""
    date_str = dt.strftime("%Y-%m-%d")
    from_str = (dt - timedelta(minutes=30)).strftime("%Y-%m-%dT%H:%M:%S")
    to_str   = dt.strftime("%Y-%m-%dT%H:%M:%S")

    # Try VIX via Polygon indices
    data = polygon_get(
        f"/v2/aggs/ticker/I:VIX/range/1/minute/{from_str}/{to_str}",
        {"adjusted": "true", "sort": "desc", "limit": 5}
    )
    if data and data.get("results"):
        vix = round(float(data["results"][0]["c"]), 1)
        print(f"[BACKTEST] VIX at {dt.strftime('%H:%M')}: {vix}")
        return vix
    ...
    if data2 and data2.get("results"):
        vix = round(float(data2["results"][0]["c"]), 1)
        print(f"[BACKTEST] VIX on {date_str}: {vix} (daily)")
        return vix

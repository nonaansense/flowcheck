...
def handle_journal_help(reply_chat_id: str):
    msg = chr(10).join([
        ...
        '  Single leg only. BTO logged automatically.',
        ...
        '  Credit spreads — LEG1=strike sold, LEG2=strike bought for protection:',
        '    /spread BE cc 07/17/26 3 465 460 1.25 @rh_brok  (sell 465C, buy 460C)',
        '    /spread NVDA cp 06/20/26 1 130 125 0.85 @rh_trad (sell 130P, buy 125P)',
        '  Debit spreads — LEG1=strike bought, LEG2=strike sold to reduce cost:',
        '    /spread AAPL dc 06/20/26 2 200 205 1.50           (buy 200C, sell 205C)',
        '    /spread SPY dp 06/20/26 1 520 515 2.00            (buy 520P, sell 515P)',
        '  Max profit/loss calculated automatically from width and premium',
        '',
        'SCREENSHOT LOGGING',
        '  Send any broker fill confirmation photo to the bot',
        '  Auto-reads: BTO/STO/STC/BTC, account type, both strikes (spreads), fill time',
        '  Single leg and spreads (credit + debit) both supported',
        '  Send multiple photos at once — each processed independently',
        '  Add @ACCOUNT in caption to override auto-detection',
        '',
        'LOGGING EXITS',
        '/exit TICKER PRICE [CONTRACTS] [DATE] [TIME]',
        ...
        '  /journal @rh_trad  IRA only',
        '/pnl [@ACCOUNT] — P&L + pattern analysis (unlocks at 10+ trades)',
        '  /pnl @rh_brok      Margin only',
        '/accounts — all accounts with P&L overview',
        '/export [@ACCOUNT] — CSV for Excel/Google Sheets',
        '  /export @rh_trad   IRA → flowcheck_journal_rh_trad.csv',
        '/debrief — AI journal analysis (needs 3+ closed trades)',
        '',
        'WEB TABLE',
        '  /journal response includes link to full sortable table',
        '  Account filter tabs + Newest/Oldest toggle',
        ...
        'DELETING',
        '/delete TICKER [@ACCOUNT]',
        '  /delete BE              (single BE trade)',
        '  /delete BE @rh_trad     (if multiple BE trades)',
        '  Deletes most recent trade for that ticker/account',
        '',
        'TAGGING',
        ...
        '  Auto-detected from screenshot account label',
        '',
...
def parse_trade_screenshot(image_bytes: bytes, caption: str = ""):
    """Use Claude Haiku vision to extract trade details from any broker screenshot."""
    ...
    print("[PHOTO] Image format: " + media_type + " size: " + str(len(image_bytes)) + " bytes")

    prompt = chr(10).join([
        "You are reading a brokerage app screenshot." + cap_note,
        "This could be an order confirmation, a filled order screen, a position screen,",
        "or any screen showing options trade details.",
        "Extract whatever trade information is visible.",
        "",
        "SPREAD DETECTION — look for ANY of these signals:",
        "  - Two different strike prices shown (e.g. 105/110, 460/465)",
        "  - Labels: Vertical, Bull Put, Bear Call, Bull Call, Bear Put,",
        "    Credit Spread, Debit Spread, Iron Condor, Iron Butterfly",
        "  - Two legs shown (Buy leg + Sell leg)",
        "  - Net debit or net credit shown",
        "If ANY of these are present: is_spread=true",
        "",
        "Return ONLY valid JSON with these fields (omit fields not visible):",
        '  "ticker": stock symbol',
        '  "expiry": MM/DD/YY',
        '  "contracts": integer',
        '  "action": "entry" or "exit"',
        '  "order_type": "BTO" | "STO" | "STC" | "BTC" (from screen text)',
        ...
        "",
        "SINGLE LEG (is_spread=false):",
        '  "strike": single strike as string',
        '  "option_type": "call" or "put"',
        '  "price": fill price per contract',
        "",
        "SPREAD (is_spread=true):",
        '  "spread_type": "credit_call" | "credit_put" | "debit_call" | "debit_put" | "iron_condor"',
        '  "short_strike": strike SOLD',
        '  "long_strike": strike BOUGHT',
        '  "option_type": "call" or "put"',
        '  "credit": net credit received OR net debit paid per share',
        "",
        "RULES:",
        "- Be GENEROUS in recognizing trade screens — if it shows any options trade info, extract it",
        "- date/time from Filled timestamp only, never guess current time",
        "- BTO/buy-to-open=entry, STC/sell-to-close=exit, STO=entry, BTC=exit",
        "- For debit spreads: spread_type=debit_call or debit_put, credit=amount paid",
        "- For credit spreads: spread_type=credit_call or credit_put, credit=amount received",
        '- Only return {"error": "not_a_trade"} if the screen has NO options trade information at all',
    ])
...
def handle_trade_photo(photo_list: list, caption: str, reply_chat_id: str):
    ...
    if data.get("error") == "empty_response":
        send_reply(
            "Could not read screenshot — image unclear or format unsupported." + chr(10) + chr(10) +
            "Try a clearer screenshot, or log manually:" + chr(10) +

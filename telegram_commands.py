...
def handle_command(text: str, from_chat_id: str):
    ...
    if cmd in ("watchlist", "watch"):
    ...
    elif cmd in ("pair_backtest", "pairbt", "backtest_pair"):
        _bt_date = args[0].strip() if args else ""
        if not _bt_date:
            send_reply('Usage: /pair_backtest YYYY-MM-DD  e.g. 2026-05-27\nStreams full day at 60x speed (~6 min). Summary + detailed alerts sent on completion.',
                       from_chat_id)
        else:
        ...
        try:
            ...
            if args and str(args[0]).lower() in ("seg", "segment", "segments", "filter"):
                send_reply("\n".join(exit_by_segment()), from_chat_id)
            else:
...
def handle_help(reply_chat_id: str):
    lines = [
        ...
        "",
        "━━ FLOW ALERTS ━━",
        "/tape — active tape watcher state (BM + retail fills today)",
        "/conviction — cross-filter conviction state (building vs fired)",
        "/retail — retail flow status  |  /retail on/off — toggle",
        "/flow TICKER — today's captured flows for a ticker",
        "/sectors — active sector cluster signals",
        "/status — stream health + today's alert count",
        "",
        "━━ POSITIONS ━━",
        "/positions — all open positions with P&L",
        "/watchlist — technical watchlist with DTE breakdown",
        "/portfolio — positions by account + sector breakdown",
        "/eval [@acct] [TICKER] — AI review: HOLD / TRIM / CLOSE",
        "/count — open position count by account",
        "/find TICKER — find position by ticker",
        "",
        "━━ RESEARCH ━━",
        "/sent TICKER — price, SMAs, RSI, news, flow, insiders",
        "/gex TICKER — GEX regime, gamma flip, call/put walls",
        "/oi TICKER STRIKE C/P EXPIRY — open interest check",
        "/oi all — OI confirmation for yesterday's alerts",
        "/price TICKER — real-time stock price",
        "/history — today's alert history link",
        "",
        "━━ JOURNAL ━━",
        "/journal — open trade journal web page",
        "/entry TICKER STRIKE C/P EXPIRY CONTRACTS PRICE — log entry",
        "/exit TICKER PRICE [CONTRACTS] — log exit",
        "/close TICKER — mark position closed",
        "/sync TICKER STRIKE C/P CONTRACTS PRICE — add position",
        "/add TICKER CONTRACTS PRICE — add to position",
        "/debrief TICKER — AI post-trade analysis",
        "/stats — win rate + P&L statistics",
        "/pnl [@acct] — P&L + slippage + fees",
        "/export [@acct] — CSV for Excel",
        ...
        "",
        "━━ ALERT RULES ━━",
        "Tape Rule A: 1+ BM + 1+ retail, same day (order doesn't matter)",
        "Tape Rule B: 2+ BM same contract, different days (no retail needed)",
        "Conviction:  1 BM + 2 retail, 6.5h retail / 7d BM window",
        "BM auto:     2+ BM same contract across days = standalone signal",
        "0DTE:        filtered everywhere — minimum 1 day to expiry",
    ]
    send_reply(chr(10).join(lines), reply_chat_id)

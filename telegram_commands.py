...
def handle_journal_help(reply_chat_id: str):
    msg = chr(10).join([
        ...
        '/delete TICKER [@ACCOUNT] — delete most recent trade for ticker',
        '  /delete BE           (if only one BE trade)',
        '  /delete BE @rh_trad  (if multiple BE trades)',
        '',
        'WEB TABLE',
...
def parse_trade_screenshot(image_bytes: bytes, caption: str = ""):
    """
    Use Claude Haiku vision to extract trade details from a broker screenshot.
    Handles single legs, credit/debit spreads, iron condors.
    ...
    prompt = chr(10).join([
        "You are reading a brokerage app screenshot (Robinhood, Tastytrade, Schwab, IBKR, Webull, TD Ameritrade)." + cap_note,
        "Extract ALL trade details from the order fill confirmation shown on screen.",
        "",
        "STEP 1 — Determine if this is a SPREAD or SINGLE LEG:",
        "  SPREAD: shows two different strikes, OR labels like 'Bull Put', 'Bear Call',",
        "  'Credit Spread', 'Debit Spread', 'Iron Condor', 'Vertical', or two separate legs",
        "  SINGLE LEG: one strike, one option bought or sold",
        "",
        "STEP 2 — Extract fields based on type:",
        "",
        "For ALL trades:",
        '  "ticker": stock symbol e.g. "DELL"',
        '  "expiry": MM/DD/YY e.g. "05/29/26"',
        '  "contracts": number of contracts (integer)',
        '  "action": "entry" or "exit"',
        '  "order_type": "BTO" (buy to open) | "STO" (sell to open) | "STC" (sell to close) | "BTC" (buy to close)',
        '  "date": fill date YYYY-MM-DD from the Filled timestamp on screen',
        '  "time": fill time as 12-hour AM/PM ET from the Filled timestamp',
        '  "account_type": account label shown e.g. "Traditional IRA" "Individual" "Margin"',
        '  "confidence": "high" | "medium" | "low"',
        "",
        "For SINGLE LEG only:",
        '  "is_spread": false',
        '  "strike": the one strike price as string e.g. "460"',
        '  "option_type": "call" or "put"',
        '  "price": fill price per contract e.g. 10.05',
        "",
        "For SPREAD only:",
        '  "is_spread": true',
        '  "spread_type": "credit_call" | "credit_put" | "debit_call" | "debit_put" | "iron_condor"',
        '  "short_strike": the strike that was SOLD (higher strike for call spread, lower for put spread)',
        '  "long_strike": the strike that was BOUGHT for protection',
        '  "option_type": "call" or "put" (underlying option type)',
        '  "credit": net credit received OR debit paid per share (not per contract)',
        "",
        "IMPORTANT RULES:",
        "- date/time MUST come from Filled timestamp on screen — never use current time",
        "- For spreads: look carefully for BOTH strikes — they are always different numbers",
        "- BTO/buy to open = entry. STC/sell to close = exit. STO = entry (short). BTC = exit (short).",
        "- If you see two strikes like 460/465 or 130/125 — that is ALWAYS a spread",
        '- Return ONLY valid JSON. If not a trade: {"error": "not_a_trade"}',
    ])
    ...
    try:
        ...
        if data.get("is_spread"):
            print("[PHOTO] SPREAD: type=" + str(data.get("spread_type")) +
                  " short=" + str(data.get("short_strike")) +
                  " long=" + str(data.get("long_strike")) +

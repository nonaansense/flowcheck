...
def handle_help(reply_chat_id: str):
    msg = chr(10).join([
        ...
        '',
        'TRADE JOURNAL',
        '/journal [@ACCOUNT] — open trades sorted by entry time',
        '/pnl [@ACCOUNT] — full P&L + pattern analysis',
...
def handle_journal_help(reply_chat_id: str):
    msg = chr(10).join([
        ...
        '',
        '  Now:          /entry BE 460 C 07/17/26 3 13.20',
        '  With time:    /entry BE 460 C 07/17/26 3 13.20 2026-05-27 10:34AM',
        '  With account: /entry BE 460 C 07/17/26 3 13.20 2026-05-27 10:34AM @RH_Brok',
        '  Screenshot:   send broker fill photo — auto-reads account + fill time',
        '  Multiple:     send several photos at once, each processed independently',
        '  Note: omitting date+time auto-fills current time with a warning',
        '',
        'LOGGING EXITS',
        '/exit TICKER PRICE [CONTRACTS] [DATE] [TIME]',
        '  Full exit:        /exit BE 18.50',
        '  Full + time:      /exit BE 18.50 2026-05-29 2:15PM',
        '  Partial (2 of 3): /exit BE 18.50 2 2026-05-29 2:15PM',
        '  Screenshot:       send broker close confirmation photo',
        '',
        'ADDING TO POSITION',
        '/add TICKER CONTRACTS PRICE [DATE] [TIME]',
        '  /add BE 1 14.00 2026-05-28 11:00AM',
        '  Blended average entry price calculated automatically',
        '',
        'SKIPPED ALERTS',
        '/missed TICKER [REASON]',
        '  /missed BE Too close to earnings',
        '',
        'VIEWING',
        '/journal [@ACCOUNT] — sorted by entry date/time, newest first',
        '  /journal           all accounts',
        '  /journal @RH_Trad  IRA account only',
        '/pnl [@ACCOUNT] — P&L + pattern analysis (unlocks at 10+ trades)',
        '  /pnl @RH_Brok      Margin account only',
        '/accounts — all accounts with P&L overview',
        '/export [@ACCOUNT] — CSV for Excel/Google Sheets',
        '  /export @RH_Trad   IRA only → flowcheck_journal_rh_trad.csv',
        '/debrief — AI analysis of journal patterns (needs 3+ closed trades)',
        '',
        'WEB TABLE',
        '  /journal response includes link to full table',
        '  Account filter tabs + Newest/Oldest sort toggle',
        '  Download button exports account-filtered CSV',
        '',
        'EDITING',
        '/edit TICKER [@ACCOUNT] FIELD VALUE',
        '  Specify @ACCOUNT when same ticker in multiple accounts',
        '  /edit BE entry_time 10:34AM',
        '  /edit BE @RH_Trad entry_time 10:34AM',
        '  Fields: entry_date  entry_time  exit_date  exit_time',
        '          entry_price  contracts  expiry  strike',
        '          note  option_type  account_id',
        '  Analytics recalculate automatically after time/price edits',
        '',
        'TAGGING',
        '/tag TICKER #tag1 #tag2',
        '  /tag BE #earnings_play #breakout',
        '  Win rate by tag in /pnl after 10+ trades',
        '',
        'ACCOUNTS — saved permanently to Supabase',
        '/account add ID NAME SIZE',
        '  /account add RH_Brok Margin 100000',
        '  /account add RH_Trad IRA 125000',
        '  Auto-detected from screenshot (Traditional IRA, Individual, etc.)',
        '  Tag entries: add @ID to /entry, or use screenshot caption',
        '',
...
def parse_trade_screenshot(image_bytes: bytes, caption: str = ""):
    ...
    prompt = chr(10).join([
        "This is a brokerage app screenshot (Robinhood, Tastytrade, Schwab, IBKR, Webull, TD Ameritrade)." + cap_note,
        "Extract ALL visible trade details from the filled order confirmation.",
        "This may be a single-leg option OR a multi-leg spread (credit spread, debit spread, iron condor).",
        'Return ONLY valid JSON. If NOT a trade confirmation, return {"error": "not_a_trade"}.',
        "",
        "JSON fields to extract:",
        '  "action": "entry" or "exit"  (BUY TO OPEN = entry, SELL TO CLOSE = exit)',
        '  "ticker": stock symbol e.g. "BE"',
        '  "strike": strike price as string e.g. "460"',
        '  "option_type": "call" or "put"',
        '  "expiry": expiry date in MM/DD/YY format e.g. "07/17/26"',
        '  "contracts": number of contracts as integer',
        '  "price": fill price per contract e.g. 13.20',
        '  "date": fill date as YYYY-MM-DD — READ FROM "Filled" or "Order Filled" timestamp on screen',
        '  "time": fill time as 12-hour AM/PM ET — READ FROM "Filled" timestamp on screen',
        '  "account_type": account label visible on screen e.g. "Traditional IRA" "Individual" "Margin" "Roth IRA" "Cash"',
        '  "confidence": "high" if all fields clearly visible, "medium" if some guessed, "low" if unclear',
        '  "is_spread": true if this is a multi-leg spread, false otherwise',
        '  "spread_type": "credit_call" | "credit_put" | "debit_call" | "debit_put" | "iron_condor" if spread',
        '  "short_strike": strike sold (if spread)',
        '  "long_strike": strike bought as protection (if spread)',
        '  "credit": net credit received or debit paid per share (if spread)',
        "",
        "CRITICAL RULES:",
        "- date and time MUST come from the Filled/Order Filled timestamp shown on screen, NOT current time",
        "- account_type MUST come from what is printed on the screen (top of screen or under Filled Order section)",
        "- If Filled timestamp shows May 27, 2026 at 10:34 AM — date=2026-05-27 time=10:34AM",
        "- BUY TO OPEN or Buy = entry. SELL TO CLOSE or Sell = exit",
        "- Omit a field only if it is completely invisible or unreadable",
        "- SPREADS: If you see TWO different strikes or a spread structure (e.g. 460/465, Bull Put, Bear Call,",
        "  Credit Spread, Debit Spread, Iron Condor) set is_spread=true and fill spread_type/short_strike/long_strike/credit",
        "- For spreads: short_strike = the strike SOLD, long_strike = the strike BOUGHT for protection",
        "- credit = net premium received (credit spread) or paid (debit spread) per share",
        "- For single leg options: is_spread=false, fill strike and price normally",
    ])
...
def handle_trade_photo(photo_list: list, caption: str, reply_chat_id: str):
    ...
    try:
        if action == "entry":
            from trade_journal import add_entry
            # Use fill date/time from screenshot — most accurate
            ...
            auto      = trade.get("entry_auto_filled", True)
            time_note = (" (fill time not found — auto-filled)" if auto
                        else " filled " + (date_str or "") + " " + (time_str or ""))
            ...
            parts = [
                "Entry logged" + conf_note + acc_label + acct_note + ":",
                ticker + " " + strike + otype + " " + expiry +

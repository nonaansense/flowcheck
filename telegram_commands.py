...
def handle_refresh(reply_chat_id: str):
    ...
    try:
        ...
        # Run pricer without market hours check
        from trade_journal import load_journal as _lj, save_journal as _sj
        from eod_pricer import get_option_quote
        from datetime import datetime
        from zoneinfo import ZoneInfo
...
def handle_journal_help(reply_chat_id: str):
    msg = chr(10).join([
        ...
        '  Credit — LEG1=strike sold, LEG2=strike bought for protection:',
        ...
        '  Debit — LEG1=strike bought, LEG2=strike sold to reduce cost:',
        ...
        '  Duplicate protection: same trade logged twice is blocked',
        ...
        '  Shows last price + unrealized P&L (updated at EOD or via /refresh)',
        '  /journal           all accounts',
        '  /journal @rh_trad  IRA only',
        '/refresh — fetch current option prices for all open positions',
        '  Updates journal + web table immediately on demand',
        '  Auto-runs at 4:02 PM ET on trading days',
        '/pnl [@ACCOUNT] — P&L + pattern + slippage + fees (unlocks at 10+ trades)',
        '  /pnl @rh_brok      Margin only',
        '/accounts — all accounts with P&L and deployed capital',
        '/export [@ACCOUNT] — CSV for Excel (account + fees + tags columns)',
        '  /export @rh_trad   IRA → flowcheck_journal_rh_trad.csv',
        '/debrief — AI journal analysis (needs 3+ closed trades)',
        '',
        'DAILY P&L',
        '  Sent automatically at 4:10 PM ET on trading days',
        '  Shows closed P&L for today + open unrealized P&L by account',
        '',
        'WEB TABLE',
        '  /journal response includes link to full sortable table',
        '  Columns: Type, Contract, Last $, Open P&L (green/red)',
        ...
        'DELETING',
        '/delete TICKER [@ACCOUNT]',
        '  /delete BE              deletes most recent BE trade',
        '  /delete BE @rh_trad     specify account if multiple BE trades',
        '',
        'TAGGING',
        ...
        '/account delete ID',
        '  /account delete rh_brok  (cannot delete if open trades assigned)',
...
def parse_trade_screenshot(image_bytes: bytes, caption: str = ""):
    ...
    prompt = chr(10).join([
        ...
        "",
        "Rules:",
        "  BTO or buy-to-open = entry. STC or sell-to-close = exit.",

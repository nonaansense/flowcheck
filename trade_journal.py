...
def add_entry(ticker: str, strike: str, opt_type: str, expiry: str,
    ...
    trade = {
        ...
        # Debit spreads:  max profit = width - debit, max loss = debit paid
        "max_profit": round(
            (float(credit or 0) * int(contracts) * 100)
            if spread_type and "credit" in (spread_type or "") else
            ((float(spread_width or 0) - float(credit or 0)) * int(contracts) * 100)
            if spread_width and credit else 0, 2
        ) if credit else None,
        "max_loss": round(
            ((float(spread_width or 0) - float(credit or 0)) * int(contracts) * 100)
            if spread_type and "credit" in (spread_type or "") else
    ...
    if open_t:
        ...
        for t in open_t:
            ...
            if t.get("is_spread") and t.get("spread_type"):
                stype  = t["spread_type"].replace("_"," ").upper()
                ...
                contract_desc = (
                    ...
                    " +" + str(ls) + "/-" + str(ss) + " x" + str(remaining) +
                    acc_label
                )
                lines.append(contract_desc)
                lines.append(

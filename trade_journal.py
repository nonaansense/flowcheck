...
def load_journal() -> dict:
    try:
        with open(JOURNAL_FILE) as f:
            data = json.load(f)
        # Migrate old journals without accounts field
        if "accounts" not in data:
            data["accounts"] = {"default": {"name": "Main", "size": 10000}}
        return data
    except:
        return {
            "trades":   [],
            "closed":   [],
            "missed":   [],
            "accounts": {"default": {"name": "Main", "size": 10000}},
        }

def get_accounts() -> dict:
    """Return all configured accounts."""
    return load_journal().get("accounts", {"default": {"name": "Main", "size": 10000}})

def add_account(account_id: str, name: str, size: float) -> dict:
    """Add or update an account."""
    journal = load_journal()
    if "accounts" not in journal:
        journal["accounts"] = {}
    journal["accounts"][account_id] = {
        "name": name,
        "size": float(size),
    }
    save_journal(journal)
    return journal["accounts"][account_id]
...
def export_journal_csv() -> str:
    ...
    headers = [
        ...
        "fc_score", "fc_verdict", "note", "is_partial", "adds_count",
    ]

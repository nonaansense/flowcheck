...
RH_CLIENT_ID   = "c82SH0WZOsabOXGP2sxqcj34FxkvfnWRZBKlBjFS"  # Robinhood web client ID
...
def login(mfa_code: str = None) -> tuple:
    ...
    try:
        print(f"[RH] Attempting login for: {username[:3]}...{username[-10:]}")
        r = requests.post(
        ...
        print(f"[RH] Login status: {r.status_code} body: {r.text[:300]}")
        data = r.json()
        ...
        if r.status_code == 200:
            ...
            print(f"[RH] 400 full response: {json.dumps(data)[:500]}")
            # MFA required
            if data.get("mfa_required") or "mfa" in str(data).lower():
                print("[RH] MFA required — check phone/email for code")
                return False, "MFA_REQUIRED"
            # Challenge required
            if "challenge" in str(data).lower():
                challenge_id = data.get("challenge",{}).get("id","")
                print(f"[RH] Challenge required: {challenge_id}")
                return False, "MFA_REQUIRED"
            detail = data.get("detail","") or data.get("username",[""])[0] if isinstance(data.get("username"), list) else str(data)
            return False, f"Login failed: {str(detail)[:200]}"

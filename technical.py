...
def add_to_watchlist(ticker: str, trade: dict, result: dict, data: dict = None):
    if ticker in _watch_list:
        print(f"[TECHNICAL] {ticker} already in watchlist — updating")
    # Store flow prices for entry quality checks
    ...
    _watch_list[ticker] = {
        ...
        "flow_stock_price": flow_stock_price,   # Stock price when flow fired
        "flow_option_price":flow_option_price,  # Option price when flow fired
        "alerted":          {},
    }
    print(f"[TECHNICAL] Added {ticker} to watchlist "
          f"({result.get('verdict')} {result.get('final_score')}/7) "
          f"stock=${flow_stock_price} option=${flow_option_price} "

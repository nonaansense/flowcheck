...
# Finnhub removed from technical scanner — 403 errors on Railway IPs
# Using Tiingo instead

def fetch_1min_candles(ticker: str, count: int = 120) -> list:
    """
    Fetch recent intraday candles via Tiingo IEX feed.
    Tiingo works from Railway IPs unlike Finnhub.
    Returns list oldest→newest.
    """
    tiingo_key = os.environ.get("TIINGO_API_KEY")
    if not tiingo_key:
        print("[TECHNICAL] TIINGO_API_KEY not set")
        return []

    try:
        # Tiingo IEX endpoint for real-time intraday data
        r = requests.get(
        ...
        if r.status_code == 200:
            ...
            if data and isinstance(data, list) and data[0].get("last"):
                ...
                # IEX gives current quote only — build synthetic candle from it
                # Use bid/ask for high/low approximation
                bid   = float(data[0].get("bidPrice") or price * 0.999)
                ask   = float(data[0].get("askPrice") or price * 1.001)
                vol   = float(data[0].get("volume") or 0)
                candle = {
                ...
                print(f"[TECHNICAL] {ticker}: ${price} via Tiingo IEX")
                return [candle]
        elif r.status_code == 404:
            print(f"[TECHNICAL] {ticker} not found on Tiingo")
        else:
            print(f"[TECHNICAL] Tiingo {r.status_code} for {ticker}")
    except Exception as e:
        print(f"[TECHNICAL] Tiingo candle error {ticker}: {e}")

    # Fallback: try Tiingo daily prices for at least price context
    try:
        ...
        if r2.status_code == 200:
            ...
            if data2 and isinstance(data2, list):
                d = data2[-1]
                price = float(d.get("close") or d.get("adjClose") or 0)
                if price > 0:
                    candle = {
                        "timestamp": int(time.time()),
                    ...
                    print(f"[TECHNICAL] {ticker}: ${price} via Tiingo daily")
                    return [candle]
    except Exception as e:
        print(f"[TECHNICAL] Tiingo daily error {ticker}: {e}")

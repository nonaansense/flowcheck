...
def fetch_price_data(ticker: str) -> dict:
    """Get current price, volume, and intraday move from Finnhub."""
    result = {}
    try:
        # Current quote
        ...
        if q:
            ...
            high       = q.get("h")
            low        = q.get("l")
            volume     = q.get("v")   # Not always available
...
def get_sentiment(ticker: str) -> str:
    ...
    lines = [
        f"📊 {ticker} Sentiment — {now_et.strftime('%b %d %I:%M%p ET')}",
        "",
    ]
    ...
    if price.get("price"):
        ...
        lines.append(
            f"{price.get('price_emoji','')} Price: ${price['price']} "
            f"({pct:+.2f}% today)"
        )
        if price.get("high") and price.get("low"):
            lines.append(
                f"   Range: ${price['low']} — ${price['high']}"
            )
    ...
    sent_label = news.get("sentiment","No data")
    lines.append(f"📰 News sentiment: {sent_label} {sent_emoji}")
    if news.get("bull_pct") is not None:
        lines.append(
            f"   Bull {news['bull_pct']}% | Bear {news['bear_pct']}% "
            f"| {news.get('articles_week',0)} articles/week"
        )
    if news.get("articles"):
        for a in news["articles"][:2]:
            h = (a.get("headline","") or "")[:70]
            lines.append(f"   · {h}")

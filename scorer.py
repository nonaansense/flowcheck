...
TIME: {data.get('time_of_day',{{}}).get('label','?')} {data.get('time_of_day',{{}}).get('emoji','')}
...
- Breakout bet = maximum WATCH"""
    ...
    for attempt in range(2):
        try:
            ...
            if (is_full_ask and is_large) or (is_mega_vol and is_full_ask):
                if verdict == "SKIP":
                    result["verdict"] = "WATCH"
                    result["final_score"] = max(result.get("final_score",0), 4.0)
                    result["reasoning"] = (result.get("reasoning","") +
                        " Upgraded to WATCH: hard rule (FULL_ASK + large premium).")

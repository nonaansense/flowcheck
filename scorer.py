...
- POST-EARNINGS (reported 0-14 days ago): 
  → Criterion 1 PASSES — catalyst already confirmed
  → IV is deflated — buying premium is CHEAPER than pre-earnings
  → Criterion 2: need 14+ days to expiry for momentum to play out
  → Look for: large EPS beat, stock already moving, continuation likely
  → This is often the BEST setup — confirmed catalyst + cheap IV
- PRE-EARNINGS (upcoming):
  → Standard rules apply — 5-14 days after earnings = sweet spot
  → IV elevated going into earnings — factor into cost
- NO EARNINGS NEAR:
  → Evaluate as insider/informed or momentum play per earlier rules

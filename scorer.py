...
5. OTM APPROPRIATE — now enhanced with ATR move analysis
   Base scoring:
   1.0 = OTM <10%
   ...
   ATR adjustment (apply on top of base):
   +0.5 = Required move ≤30% of expected range (very easy to reach)
   +0.25 = Required move ≤60% of expected range (achievable)
   -0.25 = Required move 100-150% of expected range (stretch)
   -0.5  = Required move >150% of expected range (statistically unlikely)

   IMPORTANT: Near-term expiry (DTE <14) flows with large premium often target
   small moves — a 0.5% required move with 2 DTE is EASIER than a 10% required
   move with 30 DTE even though the OTM% looks worse

   INSIDER OVERRIDE RULE:
   If fill is FULL_ASK + premium >$300K + no recent news = possible insider
   → They know a catalyst is coming that ATR cannot predict
   → SUPPRESS the ATR penalty (keep bonuses, zero out negatives)
   → Deep OTM + insider signal = lottery ticket THEY know will pay off
   → Do NOT penalize criterion 5 when atr_override=True or is_informed_flow=True
...
- Flow 10:00-11:30 AM: +0.5 criterion 6 (prime window)
- Flow 2:00-3:30 PM: +0.5 criterion 6 (afternoon conviction)
- Flow 3:30-4:00 PM, DTE <30: +1.0 criterion 6 (stealth late flow + near expiry = expects imminent move)
- Flow 3:30-4:00 PM, DTE 30-180: +0.5 criterion 6 (late flow, moderate conviction)
- Flow 3:30-4:00 PM, DTE >180 (LEAP): +0.0 (routine portfolio positioning, timing irrelevant)
- Flow after 4:00 PM, DTE <30: +1.0 criterion 6 (after-hours + near expiry = overnight catalyst expected)
- Flow after 4:00 PM, DTE >180 (LEAP): +0.0 (after-hours LEAP = no timing signal)
- LEAP definition: DTE > 180 days
- KEY RULE: Late day bonus only applies when DTE is short enough that timing actually matters
...
def score_trade(trade: dict, data: dict, pattern: dict = None) -> dict:
    ...
    mkt     = data.get("market", {})
    raw_p   = trade.get("premium", 0) or 0
    if isinstance(raw_p, str):
        s = str(raw_p).strip().upper()
        try:
            if s.endswith("M"): premium = int(float(s[:-1])*1_000_000)
            elif s.endswith("K"): premium = int(float(s[:-1])*1_000)
            else: premium = int(float(s))
        except: premium = 0
    else:
        try: premium = int(float(raw_p))
        except: premium = 0
...
MOVE ANALYSIS (ATR-based reachability):
- ATR: {data.get('atr_pct','N/A')}%/day (avg daily move)
- Required move to ITM: {data.get('move_analysis',{{}}).get('required_pct','N/A')}%
- Expected range by expiry: ±{data.get('move_analysis',{{}}).get('expected_pct','N/A')}%
- Assessment: {data.get('move_analysis',{{}}).get('assessment','Not calculated')} {data.get('move_analysis',{{}}).get('move_emoji','')}
- Score adjustment: {data.get('move_score_adj',0):+.2f}
- Use this to adjust criterion 5 score accordingly

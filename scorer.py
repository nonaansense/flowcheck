...
- Breakout bet = maximum WATCH"""
    ...
    for attempt in range(2):
        try:
            ...
            # Hard rules — minimum WATCH regardless of scorer decision
            # Rule 1: Full ask + large premium
            # Rule 2: Vol/OI >10x alone (extraordinary signal even without fill confirmation)
            # Rule 3: Large premium + Vol/OI >5x
            should_upgrade = (
                (is_full_ask and is_large) or
                (is_mega_vol) or  # Vol/OI >10x alone = minimum WATCH
                (is_large and float(vol_oi) >= 5 if vol_oi else False)
            )
            if should_upgrade and verdict == "SKIP":
                result["verdict"] = "WATCH"
                result["final_score"] = max(result.get("final_score",0), 4.0)
                reason = "Vol/OI 20x = extraordinary signal" if is_mega_vol else "FULL_ASK + large premium"

...
- Timing label: {data.get('expiry_timing_label','N/A')} {data.get('expiry_timing_emoji','')}
...
EARNINGS SCORING RULES:
- FRESH POST-EARNINGS (reported 0-7 days ago):
  → Criterion 1 PASSES — catalyst confirmed, reaction known
  → IV deflated = CHEAPEST options window
  → Need 14+ days to expiry for momentum
  → BEST setup type — score generously on timing if expiry reasonable

- POST-EARNINGS MOMENTUM (reported 8-21 days ago):
  → Criterion 1 PASSES — earnings catalyst still fresh
  → Stock still digesting the move
  → Need 10+ days to expiry
  → Good setup — score normally

- CONTINUATION PLAY (reported 22-45 days ago):
  → Criterion 1 PASSES if premium is large (>$200K) — momentum thesis
  → Far enough from earnings that this is a directional bet on trend
  → Need 20+ days to expiry
  → Evaluate on technicals and premium size

- PRE-EARNINGS (upcoming):
  → Standard rules — 5-14 days after earnings = sweet spot
  → IV elevated going in

- NO EARNINGS (>45 days or unknown):
  → Insider/informed or pure momentum — see earlier rules
...
FILL AGGRESSION — CRITICAL SIGNAL:
- Fill type: {data.get('fill_type','UNKNOWN')} {data.get('fill_emoji','')}
- Fill label: {data.get('fill_label','Unknown — check screenshot')}
- Ask %: {data.get('ask_pct','N/A')}% | Bid %: {data.get('bid_pct','N/A')}% | Multi: {data.get('multi_pct','N/A')}%

Fill type scoring impact:
- FULL_ASK / MOSTLY_ASK: Criterion 1 easier to pass — buyer paid premium aggressively
- MIXED: Neutral — evaluate on other factors
- MOSTLY_BID: Criterion 1 harder to pass — passive fill, less conviction signal
- SPREAD_LEG: Flag as possible hedge — reduce confidence in directional thesis
- UNKNOWN: Note gap, score conservatively on criterion 1
...
Never say "wait for complete data" as a suggestion — give actionable trade advice."""
    ...
    try:
        ...
        should_retry = (
            ...
            "500" in err_str or
            "internal server" in err_str.lower() or
            "overloaded" in err_str.lower()
        )
        if should_retry:
            wait = 45 if "rate_limit" in err_str else 15
            print(f"[SCORER] API error ({err_str[:50]}) — waiting {wait}s then retrying")
            time.sleep(wait)
            try:
                response = client.messages.create(
                    model="claude-sonnet-4-5",

...
def save_flow_history(history: list):
    try:
        with open(FLOW_HISTORY_FILE,"w") as f:
            json.dump(history[-500:], f)  # Cap at 500 entries
    except Exception as e:
        print(f"[INTEL] Flow history save error: {e}")
...
def detect_repeat_buyer(trade: dict, history: list) -> dict:
    ...
    if len(unique_days) >= 2:
        total_premium = sum(f.get("premium",0) or 0 for f in matches)
        return {
            "is_repeat":      True,
            "repeat_emoji":   "🔁",
...
def track_sector_flow(trade: dict, result: dict) -> dict | None:
    ...
    if flow_count >= 3:
        ...
        total_premium = sum(f.get("premium",0) or 0 for f in flows)

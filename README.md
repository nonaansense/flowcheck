...
FlowCheck supports two flow sources, switchable via `FLOW_SOURCE` Railway variable:

### FlowGod Mode (default)
```
FLOW_SOURCE = flowgod
```
- Monitors @FL0WG0D on X via IFTTT
- Pre-curated — FlowGod filters before posting
- Lower volume (3-8 alerts/day), higher signal quality
- Webhook: IFTTT → POST /webhook

### Bullflow Mode
```
FLOW_SOURCE = bullflow
```
- Direct Bullflow.io webhook integration
- Raw order flow — everything hits, pre-filter runs first
- Higher volume (50+ alerts/day), FlowCheck filters to 2-5 high conviction
- Webhook: Bullflow → POST /webhook-bullflow

Configure Bullflow webhook URL:
```
https://web-production-19e44.up.railway.app/webhook-bullflow
```

Check current source and filter settings:
```
https://web-production-19e44.up.railway.app/flow-source
```

### Bullflow Pre-Filter (hard rules — auto-reject if any fail)

| Filter | Default | Variable |
|--------|---------|----------|
| Min premium | $150,000 | `FILTER_MIN_PREMIUM` |
| Min open interest | 500 | `FILTER_MIN_OI` |
| Min DTE | 7 days | `FILTER_MIN_DTE` |
| Max DTE | 90 days | `FILTER_MAX_DTE` |
| Max OTM | 20% | `FILTER_MAX_OTM` |
| No entries before | 10:00 AM ET | hardcoded |
| No entries after | 3:30 PM ET (unless DTE<30) | hardcoded |
| No earnings within | 2 days | hardcoded |
| Fill type | Reject AT_BID / BELOW_BID | hardcoded |
| Chasing | Reject if option up >50% | hardcoded |

Survivors then go through the full 7-point Claude scoring pipeline.
...
| `daily_pnl.py` | Daily P&L summary at 4:10 PM ET — closed + unrealized |
| `eod_pricer.py` | EOD option pricer — fetches closing prices for open positions at 4:02 PM ET |
...
| `FLOW_SOURCE` | flowgod | Flow source: `flowgod` or `bullflow` |
| `FILTER_MIN_PREMIUM` | 150000 | Bullflow: minimum premium ($) |
| `FILTER_MIN_OI` | 500 | Bullflow: minimum open interest |
| `FILTER_MIN_DTE` | 7 | Bullflow: minimum days to expiry |
| `FILTER_MAX_DTE` | 90 | Bullflow: maximum days to expiry |
| `FILTER_MAX_OTM` | 20.0 | Bullflow: maximum OTM % |
...
Screenshot entry auto-detects account type, fill time, fees, and both spread strikes.
Duplicate protection prevents the same trade being logged twice from the same screenshot.
Position sizing shown on every entry — deployed capital and remaining buying power.
View P&L per account with `/pnl @accountid` or overview with `/accounts`.

`/pnl` also shows slippage vs flow alert price and total fees paid.
